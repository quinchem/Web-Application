...
// Dữ liệu bài viết cần edit (truyền từ controller)
$editPost      = $post    ?? [];
$editPostId    = $editPost['post_id']      ?? '';
$editTitle     = $editPost['title']        ?? '';
$editSummary   = $editPost['summary']      ?? '';
$editContent   = $editPost['content']      ?? '';
$editThumb     = $editPost['thumbnail_URL'] ?? '';
$editStatus    = $editPost['status']       ?? 'draft';
$editCategoryId = $editPost['category_id'] ?? '';

// Tags đã có (mảng string, truyền từ controller qua $tags)
$editTags = $tags ?? [];

// Tìm parent_id của category hiện tại
$editParentId = '';
foreach (($categories ?? []) as $cat) {
    if ((string)$cat['category_id'] === (string)$editCategoryId && !empty($cat['parent_id'])) {
        ...
        <div class="create-post-wrapper">
            <div class="create-post-main">
                ...
                <div class="create-post-heading">
                    ...
                    <div class="subtitle">Cập nhật nội dung bài viết của bạn</div>
                </div>
                ...
                <div class="post-count-label" id="pcDraftLabel">
                    <?= htmlspecialchars($editTitle ?: 'Bài viết nháp') ?>
                </div>

                <form method="POST" action="index.php?page=client_store_post"
                    enctype="multipart/form-data" id="pcPostForm">

                    <!-- post_id luôn có giá trị → controller sẽ UPDATE -->
                    <input type="hidden" name="post_id" id="pcPostId"
                        value="<?= htmlspecialchars($editPostId) ?>">

                    <!-- Giữ thumbnail cũ nếu không upload mới -->
                    <input type="hidden" name="existing_thumbnail" id="pcExistingThumbnail"
                        value="<?= htmlspecialchars($editThumb) ?>">
                    ...
                    <div class="pc-card">
                        <input type="text"
                            ...
                            autocomplete="off"
                            value="<?= htmlspecialchars($editTitle) ?>"
                            required>
                        ...
                        <textarea name="summary"
                            ...
                            rows="3"><?= htmlspecialchars($editSummary) ?></textarea>
                    </div>
                    ...
                    <div class="pc-card">
                                    ...
                                    <select name="parent_category"
                                        ...
                                        <?php foreach ($catParents as $parentId => $parent): ?>
                                            <option value="<?= htmlspecialchars($parentId) ?>"
                                                <?= (string)$parentId === (string)$editParentId ? 'selected' : '' ?>>
                                            ...
                                            <?php if (!empty($catChildren[$parentId])): ?>
                                                <?php foreach ($catChildren[$parentId] as $child): ?>
                                                    <option value="<?= htmlspecialchars($child['category_id']) ?>"
                                                        ...
                                                        <?= (string)$child['category_id'] === (string)$editCategoryId ? 'selected' : '' ?>
                                                        style="<?= (string)$parentId !== (string)$editParentId ? 'display:none;' : '' ?>">
                        ...
                        <div id="pcTagsHidden">
                            <?php foreach ($editTags as $tag): ?>
                                <input type="hidden" name="tags[]"
                                    value="<?= htmlspecialchars(is_array($tag) ? ($tag['name'] ?? '') : $tag) ?>">
                            <?php endforeach; ?>
                        </div>
                        ...
                        <div class="pc-status-row">
                            ...
                            <span class="pc-status-badge <?= $statusClass ?>" id="pcStatusBadge">
                                <?= $statusLabel ?>
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="pc-card">
                        ...
                        <div class="pc-thumbnail-zone" id="pcThumbnailZone">
                            ...
                            <img id="pcThumbnailPreview"
                                src="<?= htmlspecialchars($editThumb) ?>"
                                alt=""
                                style="<?= empty($editThumb) ? 'display:none;' : '' ?>">
                        </div>
                    ...
                    <div class="pc-card">
                        <div class="pc-content-toolbar">
                            ...
                            <button type="button" onclick="pcFormat('formatBlock','blockquote')" title="Trích dẫn" style="font-size:.75rem;">99</button>
                            <div class="tb-sep"></div>
                            <button type="button" onclick="pcFormat('insertUnorderedList')" title="Danh sách"><i class="fa-solid fa-list-ul"></i></button>
                            <div class="tb-sep"></div>
                        ...
                        <!-- Content gốc truyền cho JS fill vào editor -->
                        <textarea name="content" id="pcContentInput" hidden><?= htmlspecialchars($editContent) ?></textarea>
                    ...
                    <div class="pc-action-bar">
                        ...
                        <button type="button" id="pcCancelBtn" class="pc-btn-draft">
                            Huỷ chỉnh sửa
                        </button>
...
<div id="pcCancelModal" class="pc-modal-overlay" style="display:none;">
    <div class="pc-modal-box">
        ...
        <h3>Huỷ chỉnh sửa?</h3>
        <p>Các thay đổi chưa lưu sẽ <strong>bị mất</strong>. Bạn có chắc muốn quay lại?</p>
        <div class="pc-modal-actions">
            <button type="button" id="pcCancelModalNo" class="pc-btn-cancel">Không, tiếp tục sửa</button>
            <button type="button" id="pcCancelModalYes" class="pc-btn-publish" style="background:var(--red);">Thoát không lưu</button>
        </div>
    </div>
</div>

<!-- Truyền dữ liệu bài viết sang JS -->
<script>
    window.PC_EDIT_DATA = {
        postId   : <?= json_encode($editPostId) ?>,
        tags     : <?= json_encode(array_map(fn($t) => is_array($t) ? ($t['name'] ?? '') : $t, $editTags)) ?>,
        content  : <?= json_encode($editContent) ?>,
        hasThumb : <?= json_encode(!empty($editThumb)) ?>,
        isEdit   : true
    };
</script>

<script src="https://cdn.jsdelivr.net/npm/jquery@3.7.1/dist/jquery.min.js"></script>
<script src="Public/Client/Js/clientPostEdit.js"></script>

    ...
    fetch('index.php?page=api_comment', {
        ...
        .then(data => {
            ...
            if (data.status === 'success') {
                ...
                // 2. Chèn bình luận mới vào đầu danh sách
...
document.addEventListener('click', function (e) {
    ...
    const form = replyBtn.closest('.w-100').querySelector('.reply-form');
...
document.addEventListener('click', function (e) {
    ...
    const parentId = submitBtn.dataset.commentId;
    ...
    fetch('index.php?page=api_comment', { method: 'POST', body: formData })
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                const replyHtml = `
                <div class="d-flex mt-3 ms-4 pb-3 border-bottom reply-item">
                    <img src="${data.comment.avatar}" class="comment-avatar me-3" alt="">
                    <div class="w-100">
                        <div class="d-flex justify-content-between mb-1">
                            <div class="fw-bold" style="color:var(--navy);">${data.comment.full_name}</div>
                            <div class="small text-muted">${data.comment.created_at}</div>
                        </div>
                        <div style="font-size:0.95rem;line-height:1.6;">${data.comment.content}</div>
                    </div>
                </div>`;

                form.insertAdjacentHTML('afterend', replyHtml);
                Swal.fire({
                    icon: 'success', toast: true, position: 'top-end',
                    showConfirmButton: false, timer: 1200, title: 'Đã gửi trả lời'
                });
            }
        });
});